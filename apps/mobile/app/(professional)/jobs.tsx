...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  confirmed: { label: 'Confirmado', color: COLORS.success, bg: COLORS.successLight },
};
...
export default function ProfessionalJobsScreen() {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtitle}>
        {activeTab === 'active'
          ? 'Cuando un cliente acepte tu propuesta, el trabajo aparecerá acá.'
          : 'Los trabajos confirmados por el cliente aparecerán acá.'}
      </Text>
    </View>
  );
  ...
  const renderJobCard = ({ item }: { item: JobItem }) => {
    ...
    const clientName = item.users?.name ?? 'Cliente';
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
        onPress={() => router.push(`/(professional)/job/${item.id}`)}
      >
        <View style={styles.cardHeader}>
          ...
          <View style={styles.cardInfo}>
            <Text style={styles.cardClientName} numberOfLines={1}>{clientName}</Text>
            {categoryName ? (
