  ...
  useEffect(() => {
    async function load() {
      const { data: prof } = await supabase
        .from('professionals')
        .select('balance_due, jobs_completed, rating_avg')
        .eq('user_id', user?.id)
        .single();
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
      ...
      <View className="flex-row px-4 mt-4 gap-3">
        <View className="flex-1 bg-white rounded-card p-4 items-center">
      ...
      <View className="px-4 mt-6">
        ...
        {payments.length === 0 ? (
          ...
          <View className="gap-3">
            {payments.map((pay) => (
              <View key={pay.id} className="bg-white rounded-card p-4">
